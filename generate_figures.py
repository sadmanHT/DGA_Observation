...
for rep,c,m,ls,label in [("Statistical_28",NAVY,"o","-","Statistical-28"),("Temporal_82",MID,"s","--","Temporal-82")]:
    d=df[df.representation==rep].sort_values("horizon_percent")
    ax.plot(d.horizon_percent,d.test_macro_f1,color=c,marker=m,ls=ls,lw=1.35,ms=3.3,markevery=2,label=label)
...
for off,h,c in zip([-bh,0,bh],[50,75,100],[LIGHT,MID,NAVY]):
    vals=[float(d[(d.horizon_percent==h)&(d.group==f)].share_of_total_importance.iloc[0]) for f in fams]
...
print("Vector manuscript figures written to", OUT)
